...
def check_differences(beamline, year, visit, folder=None):
    ...
    if folder:
        ...
        if not os.path.exists(raw_location):
            logger.debug(f'This folder {raw_location} does not exist!')
            logger.debug('The expected format for folder is Merlin/sample1/dataset1/')
            sys.exit()
    else:
    ...
    logger.debug(f'processing path: {proc_location}')
    ...
    # logger.debug(f'converted_dirs: {converted_dirs}')
    # only using the time-stamp section of the paths to compare:
    raw_dirs_check = []
    converted_dirs_check = []
    # logger.debug(f'**** mib_paths {mib_paths}')
    # logger.debug(f'****hdf_files {hdf_files}')
    ...
    # logger.debug(f'raw_check {raw_dirs_check}')
    # logger.debug(f'converted_check {converted_dirs_check}')
    to_convert_folder = set(raw_dirs_check) - set(converted_dirs_check)
    # logger.debug(to_convert_folder)
    mib_to_convert = []
